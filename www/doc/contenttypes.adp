...
<body>
<h2>ETP Content Types</h2><br clear="right"><a href="./">ETP Documentation</a>:ETP Content Types
<h3>Standard page attributes</h3>
The content repository data model (a standard part of OpenACS 4)
primarily keeps track of <i>content items</i>, each of which may
have multiple <i>content revisions</i>. The
<code>content_revision</code> object type refers to a row in the
<code>cr_revisions</code> table. Each revision contains the
standard attributes of the pages you create with ETP, such as
Title, Description, and Content. Additionally, the standard
<code>acs_object</code> attributes are stored for each revision,
...
<code>cr_revisions</code> table.</p><h3>Extended page attributes: why do we need them?</h3>
...
findings which can be skimmed or searched).</p><p>All the standard page attributes (Title, Description, Content,
...
to have a section and an abstract.</p><h3>Defining a new content type</h3>
Creating a new content type is done by calling the <code>
<b style="color: black; background-color: rgb(255, 255, 102);">etp</b>::define_content_type</code>
...
The first 3 parameters to <code>define_content_type</code> are the
...
control. useful attributes are "size=X" to specify the size of
standard input controls, and "rows=X cols=X" to specify the size of
a textarea. Textareas will be used only if the datatype is string
and html specifies rows or cols.)</li><li>default_value (can either be a string denoting a single default
value, or the name of a callback function you've defined in the etp
namespace which is used to provide values for select lists).</li>
</ul>
Once you've defined a content type, you may refer to it when
calling the <code>etp::define_application</code> procedure to set
...
is the subject of the <a href="templates">next page</a>. After
that's been done, the authors of the journal will be able to create
a new issue of the journal simply by creating a new instance of the
ETP package (a process that's automated within the ETP interface by
the "create subtopic" command) and ensuring that the new content
section is using the journal application. This setup can be
automated, since it's possible to specify the application to use
for any subtopic created within a particular directory.
<hr><table width="100%"><tbody><tr><td><address><a href="mailto:luke\@museatech.net">luke\@museatech.net</a></address></td></tr></tbody></table>
</body>
