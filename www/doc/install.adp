...
<body>
<h2>Installation instructions</h2><br clear="right"><a href="./">ETP Documentation</a>:Install
...
package manager (found at <code>acs-admin/apm</code>), and
...
to restart your server to load all of the package's tcl code.</p><p>Now you may test the package by visiting the Site Map (found at
...
content of the page.</p><p>However, you're not really having fun until you can modify your
...
redirected to the Edit this page instance (/intranet).</p><p>Or, if you want requests for (/) to still go to /, then by
...
remains mounted at the top level.</p><blockquote><pre>
cd /web/MYSERVER/www
mkdir index-backup
mv index* index-backup
ln -s ../packages/edittthispage/www/index.vuh .
</pre></blockquote><hr><table width="100%"><tbody><tr><td><address><a href="mailto:luke\@museatech.net">luke\@museatech.net</a></address></td></tr></tbody></table>
</body>
